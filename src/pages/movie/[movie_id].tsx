    ...
    <>
      ...
      {data && data.detail ? (
        <main className="min-h-screen">
            ...
            <div className="container relative z-10 py-10 mx-auto">
              <div className="flex gap-6">
                <img
                  ...
                  alt=""
                />
                <div className="w-[calc-370px] text-white px-10">
                  <h1 className="text-[36px] font-bold">{data.detail.title}</h1>
                  <div className="flex items-center gap-1 mt-2">
                    <Link href={data.detail.homepage} passHref target="_blank">
                      <AiOutlineGlobal size={30} className="mr-2" />
                    </Link>
                    <FaCalendar />
                    {data.detail.release_date}
                    {data.detail.genres.map((item, key) => (
                      <p key={item.name + key}>, {item.name}</p>
                    ))}
                  </div>
                  <div className="my-5">
                  ...
                  <p className="my-1 text-lg capitalize">
                    status:{" "}
          ...
          {data && data.credits && (
            <div className="container mx-auto max-w-[1280px] pt-10 pb-20">
